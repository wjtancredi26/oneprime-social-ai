...
function networkRequiresImage(network) {
...
export function buildPostCreateData({
  ...
  if (normalizedImageUrl && !isPublicHttpsUrl(normalizedImageUrl)) {
    throw new Error("A imagem precisa estar em uma URL pública HTTPS.");
  }

  if (networkRequiresImage(network) && !normalizedImageUrl) {
    throw new Error("Postagens para Instagram exigem uma imagem pública.");
  }
